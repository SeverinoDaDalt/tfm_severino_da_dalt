...
\begin{scope}[xshift=11 cm]
    ...
    \end{tikzpicture}
    \caption{\emph{On the left}, an example of a graph, smaller than a $3\times3$ half-graph, for which no induced copies can be found in a $3$-stable graph. It is basically a $3\times3$ half-graph in which $a_1$ and $b_2$ are the same vertex, and an edge is added between $a_2$ and $a_3$.
