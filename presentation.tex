...
% SzRL
\begin{frame}{Introduction to Szemerédi's Regularity Lemma}
    \begin{theorem}<1->[Szemerédi's Regularity Lemma, \cite{regular_partitions_of_graphs}]
...
\begin{frame}{Introduction to Szemerédi's Regularity Lemma}
    ...
    \begin{enumerate}
        \item The bound on the number of parts $M$ is a \emph{tower-type} function of $\epsilon$,
            ...
            This is unavoidable, as shown by Gowers~\cite{lower_bounds_of_tower_type_for_szeremedis_uniformity_lemma}.
...
\begin{frame}{Property Testing of $H$-freeness in Stable Graphs}
    ...
    \visible<3-> {
        Alon et al.~\cite{efficient_testing_of_large_graphs} proved that $H$-freeness is testable for every $H$.
    }\visible<4-> {Later, Alon and Shapira~\cite{a_characterization_of_the_natural_graph_properties_testable_with_one_sided_error}
