\section{Section 2} \label{sec:section_2}
    ...
    \definition\label{k-tree}
        ...
        if $\eta \frown \left< \ell \right> \triangleleft \rho$, then $\left( b_\rho R c_\eta \right) \equiv \left( \ell = 1 \right)$.
    ...
    \theorem\label{tree_implies_order}
        If a graph $G$ has tree bound at least $k_{**} = 2^{k_*+1}-2$, then it has the $k_*$-order property.
        \begin{proof}
            ...
            \begin{claim}
                ...
                \begin{proof} \it{(of claim)}
                    ...
                    Let $Z_i = \parcurly{c_\eta \mid \eta \in [2]^{<k'}, \partriangle{i} \triangleleft \eta}$.
            ...
            the sequence resulting by replacing $H$ in (\ref{itm:tree_implies_order.1}) by $b_*$, $c_*$ is a $k_*$-order.
            ...
            We have two cases:
            \begin{itemize}
                \item \emph{Case 1.} There is a branch $b_*$ such that $Z(b_*)$ contains an $(h+1)$-tree.
                    In that case, we can take $c_*$ to be the top node of the $(h+1)$-tree, and $H'$ to be its
                    "disconnected"-side $h$-subtree. \todo{"Disconnected side" can be formalized}
                    Replacing $H$ in (\ref{itm:tree_implies_order.1}) with $H'$, $b_*$, $c_*$ in this order, the
                    conditions for $S_{r-1}$ are satisfied.
                \item \emph{Case 1.} There is no branch $b$ such that $Z(b)$ contains an $(h+1)$-tree.
                    Now, let $c_*$ be the top node of $H$, $Z_1$ the set of nodes in the "connected"-side of $H$, and
                    $b_*$ any branch of the "connected"-side of $H$. \todo{"Connected side" can be formalized}
                    By the assumption of this case, $Z(b) \cap Z_1$ contains no $h+1$-tree, so by the claim,
                    $Z_1 \setminus Z(b)$ contains an $h$-tree.
                    Finally, replacing $H$ in (\ref{itm:tree_implies_order.1}) by $b_*$, $c_*$, $H'$ in this order, the
                    conditions for $S_{r-1}$ are satisfied.
            \end{itemize}
        \end{proof}