\section{Section 3} \label{sec:section_3}
    ...
    Stable graphs are graphs which do not contain \say{quasi-induced}\todo{Lluis: what is the right term here?}
    large half-graphs, a particularly \say{irregular} structure in graphs. \todo{Mention how different papers call this differently.}
    See \Cref{pic:half_graph} for an example of such graph. \todo{Add visual example of a half-graph}
    ...
    \begin{remark}
        The two sequences of vertices in the previous definition need not to be disjoint.
        Thus, we are not only excluding just half-graphs, but a larger class of subgraphs.
    ...
    In order to classify what is the expected behaviour in a graph, or more more specifically, in a pair of sets of vertices,
    ...
    Then, we say that a vertex $a \in A$ is \emph{exceptional} with respect to $B \subseteq G$ if $t(a,B) \not\equiv t(A,B)$,
    ...
    \begin{itemize}
        ...
        \item $B^+_{A,b} = \parcurly{a \in A \mid a R b}$, the vertices of $A$ connected to $b$.
            \todo{Maybe, move these two to the lemma where they are needed.}
    ...
    A useful tool to deal with them is \Cref{lem:k_order_propery_bounds_BAbs}, which gives a bound on the number of such sets under the
    ...
    We first prove a stronger version of this lemma from Pajor, for which Sauer-Shelah will be a straightforward
    consequence.
    ...
    \todo{Lluis: pot semblar raro aixó, pero crec que aixi deixa clar més endavant on de veritat necessitem que siguin
    ...
    \begin{lemma}[Claim 2.6] \label{lem:k_order_propery_bounds_BAbs}
        ...
        \begin{enumerate}
            \item First of all, notice that $B^+_{A,b} = B - B^-_{A,b}$, since by definition they are complementary.
                ...
                \[
                    ...
                    \parstraight{\parcurly{B^+_{A,b} \mid b \in G}} \leq |A|^k
                \]
                where the last inequality follows \Cref{lem:k_order_propery_bounds_BAbs}.
    ...
    \begin{theorem} \label{thm:tree_implies_order}
        ...
        \begin{proof}
            ...
            there exists a map $f \colon \parcurly{0,1}^{<k'} \longrightarrow \parcurly{0,1}^{<k}$ such that for all $\eta, \eta' \in \parcurly{0,1}^{<k'}$,
