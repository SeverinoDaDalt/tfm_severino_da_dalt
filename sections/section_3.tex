\section{Stable Graphs} \label{sec:section_3}
    ...
    Note that the vertex sets $\parcurly{a_i}_i$ and $\parcurly{b_i}_i$ are \say{ordered} according to their neighbourhood,
    ...
    \subsection{$k$-order Property} \label{subsec:subsection_3.1}
        ...
        \begin{remark}
            Notice that the vertices within each sequence $\parcurly{a_i}_i$, $\parcurly{b_i}_i$ must be distinct,
            ...
            $\parcurly{a_i}_i$, $\parcurly{b_i}_i$, and possibly identifying some pairs of vertices $(a_i, b_j)$).
