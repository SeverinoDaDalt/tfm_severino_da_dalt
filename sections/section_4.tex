\section{Section 4} \label{sec:section_4}
    ...
    Next, we will follow another approach.
    That is, if our \regular property is transferred to subsets, then creating an equitable partition is not a hard
    problem, as we can simply choose a subset of the obtained part to be of the desired size.
    A very simple such property is $f_c$-indivisibility, where $f_c$ is the constant function $f_c(x) = c$.
    That is, the number of exceptional edges is at most $c$ for each vertex of the graph, thus taking a subset cannot
    introduce new exceptions.
    This property is much stronger then $\epsilon$-indivisibility, \jojo
    ...
    \begin{theorem}[Theorem 4.23] \label{thm:equitative_partition_high_regularity_parts_grow_with_n}
        ...
        \begin{itemize}
            ...
            \item $|B| < m_0$. \todo{$m_0$ cannot be the bound as it is not defined in the statement. There is no clear
