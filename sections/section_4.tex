\section{Section 4} \label{sec:section_4}
    ...
    \begin{lemma}[Claim 4.3] \label{lem:existance_of_indivisible_sets}
        ...
        \begin{proof}
            ...
            Thus, there exists $b_\eta$ such that $A_\eta^{(i)} \geq f(m_{k-1}) \geq m_{k}$ (\dref{itm:existance_of_indivisible_sets.4}), and we can choose
            $A_{\eta^\frown \partriangle{i}} \subseteq A_\eta^{(i)}$ (\dref{itm:existance_of_indivisible_sets.5}), such that
            $|A_{\eta^\frown \partriangle{i}}| = m_{k}$ $\forall i \in \parcurly{0,1}$ (\dref{itm:existance_of_indivisible_sets.3}).
