\section{Section 4} \label{sec:section_4}
    ...
    All in all, even though the partitions of this section present a very strong form of \say{regularity}, they all share
    ...
    The following lemma measures precisely that, although doing so in a \say{directed} way.
    ...
    Next, we are interested in studying how the average condition of an indivisible pair controls the homogeneity of
    large enough subpairs, in the sense of bounding exceptional edges.
    ...
    \begin{remark}\label{rmk:sufficient_requirement_for_average_condition}
        Notice that the average condition is easily satisfied if a large enough condition is met by the pair.
    ...
    Now that we have proven the nice properties of indivisible sets, we are actually interested in whether they can be
    found in a graph.
    ...
    The previous proof can be iteratively be used to partition the graph into indivisible parts with a small reminder.
    ...
    Finally, we can ensure the pairs satisfy the average condition by simply ensuring a minimal size of the parts,
    which can be easily implemented in the sequence of integers.
    ...
    \begin{lemma}[Claim 4.13] \label{lem:bound_on_the_probability_of_a_subpair_having_no_exceptions}
        ...
        paper, but required for the calculations (sin hacer trampa).} and for all $\ell \in \parcurly{0, \dots, k_{**}-1}$, $\floor{m_\ell^\epsilon} = m_{\ell+1}$,
        for some $\epsilon \in (0, \frac{1}{2})$ such that $2 < (m_{k_{**}-1})^{1-2\epsilon}$.
        ...
        It can be enforced because the only lemma that uses it actually suppose it in the construction.}
        ...
        \begin{proof}
            ...
            It follows the condition $2 < (m_{k_{**}-1})^{1-2\epsilon}$ and \Cref{rmk:sufficient_requirement_for_average_condition}
    ...
    \begin{theorem}[Theorem 4.16] \label{thm:existance_of_equitative_partition_with_perfect_pairs_but_with_bound_exceptional_pairs}
        ...
        \begin{proof}
            ...
            \begin{enumerate}
                ...
                \item $m_{**} \leq n^{\frac{1-\epsilon-c}{3}\epsilon^{k_{**}}}$.
    ...
    \begin{remark}
        \jojo
        \todo{Things to be discussed in the remark: \\
            - the upperbound on $m_{**}$ is close to a more reasonable value. \\
            - in any case, the window of choice for $m_{**}$ is not that large. \\
            - $c$ closer to $0$ increases the number of exceptional pairs but decreases size of the remainder, and viceversa.
            }
    \end{remark}
    ...
    \begin{lemma}[Claim 4.21] \label{lem:many_values_to_equitative_partition_with_bound_exceptional_pairs}
        ...
        \frac{\xi}{\epsilon^{k_{**}}}, c}$ in the sense of \Cref{lem:n_large_enough_valid_values} (d)), if
        ...
        \begin{proof}
            ...
            So $m_{\ell+1} = m_\ell^\epsilon = \lfloor (m_\ell)^\epsilon \rfloor$ and we can use \Cref{lem:existance_of_indivisible_sets}
            to get $A_1 \subseteq A$ with $|A_1| = m_\ell$ for some $\ell \in \parcurly{0, \dots, k_{**}-1}$ and $A_1$ $\epsilon$-indivisible.
            By \dref{itm:many_values_to_equitative_partition_with_bound_exceptional_pairs.2} we have that $|P| \leq |A_1|^k = m_\ell^k$.
            \todo{Define $P$ in this context.}
            Notice that:
            \begin{itemize}
                \item $\epsilon \in (0,1)$ by \dref{itm:many_values_to_equitative_partition_with_bound_exceptional_pairs.3}.
                \item $\zeta \coloneqq \frac{1}{k} > 0$.
                \item since $\epsilon \in \parround{0, \frac{1}{2}}$ by \dref{itm:many_values_to_equitative_partition_with_bound_exceptional_pairs.3},
                    then by \dref{itm:many_values_to_equitative_partition_with_bound_exceptional_pairs.4}
    ...
    \begin{remark}[Remark 4.22] \label{rmk:k_asterisk_enough_for_k}
        Notice that if $k = k_*$, the condition \dref{itm:many_values_to_equitative_partition_with_bound_exceptional_pairs.2} will be satisfied by \Cref{cor:k_order_propery_bounds_BAbs}
    ...
    \begin{theorem}[Theorem 4.23] \label{thm:equitative_partition_high_regularity_parts_grow_with_n}
        ...
        $A = n > n\parround{c, \epsilon, \zeta, \xi, \theta}$ (i.e. $n$ large enough in the sense of \Cref{lem:n_large_enough_valid_values}),
        \todo{Change this notation to use $N_{\ref{lem:n_large_enough_valid_values}}$.}
        ...
        \begin{proof}
            ...
            in the sense of \Cref{lem:n_large_enough_valid_values}, so that $\lfloor n^\theta \rfloor$ satisfies the
            large enough condition of \Cref{lem:many_values_to_equitative_partition_with_bound_exceptional_pairs}:
            \[
                \parround{\lfloor n^\theta \rfloor}^{\epsilon^{k_{**}}}
                    > n\parround{\epsilon, \frac{1}{k_*}, \frac{\xi}{\epsilon^{k_{**}}}, c}
            \]
            Notice that condition \dref{itm:many_values_to_equitative_partition_with_bound_exceptional_pairs.2} in
            \Cref{lem:many_values_to_equitative_partition_with_bound_exceptional_pairs}
            is satisfied by \Cref{rmk:k_asterisk_enough_for_k}.
