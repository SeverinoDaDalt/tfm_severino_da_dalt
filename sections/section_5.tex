\section{The Stable Regularity Lemma} \label{sec:section_5}
    ...
    \subsection{Excellent partitions} \label{subsec:subsection_5.2}
        ...
        \begin{lemma}[Claim 5.13 in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:even_partition_of_excellent_parts}
            ...
            \begin{proof}
            \begin{enumerate}[label=(\alph*), ref=\alph*]
                \item For each $b \in G$, we say that $B_{A,b}$ is \emph{bad} if $\parstraight{B_{A,b}} \geq \epsilon m$.
                    For each bad $B_{A,b}$, let $X_{A,b,A'}$ be the event that
                    $\parstraight{B_{A,b} \cap A'} \geq \parround{\epsilon + \zeta} m$ for a subset
                    $A' \subseteq A$ of size $m$, sampled uniformly at random.
                    Notice that $\parstraight{B_{A,b} \cap A'}$ is modelled by a hypergeometric distribution,
                    ...
                    That is, $X = \sum_{\text{bad } B_{A,b}} \mathbbm{1}_{X_{A,b,A'}}$, where $\mathbbm{1}_Y$ is the
                    indicator random variable of the event $Y$.
                    We compute the expectation
                    \[
                        \mathbb{E}[X] = \sum_{\text{bad } B_{A,b}} \mathbb{E}[\mathbbm{1}_{X_{A,b,A'}}]
                            = \sum_{\text{bad } B_{A,b}} P\parround{X_{A,b,A'}}
                            \leq \sum_{\text{bad } B_{A,b}} e^{-2\zeta^2 m}.
                    \]
                    Following \dref{itm:k_order_propery_bounds_BAbs.2} in \Cref{cor:k_order_propery_bounds_BAbs}, the number of intersections of bad $B_{A,b}$'s with $A'$, can be bounded
                    by $m^{k_*}$.
