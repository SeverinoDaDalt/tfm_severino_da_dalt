\section{The Stable Regularity Lemma} \label{sec:section_5}
    ...
    \subsection{Excellent partitions} \label{subsec:subsection_5.2}
        ...
        \begin{lemma}[Claim 5.13 in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:even_partition_of_excellent_parts}
            ...
            \begin{proof}
            \begin{enumerate}[label=(\alph*), ref=\alph*]
                \item For each $b \in G$, we say that $\overline{B}_{A,b}$ is \emph{bad} if $\parstraight{\overline{B}_{A,b}} \geq \epsilon m$.
                    For each bad $\overline{B}_{A,b}$, let $X_{A,b,A'}$ be the event that
                    $\parstraight{\overline{B}_{A,b} \cap A'} \geq \parround{\epsilon + \zeta} m$ for a subset
                    $A' \subseteq A$ of size $m$, sampled uniformly at random.
                    Notice that $\parstraight{\overline{B}_{A,b} \cap A'}$ is modelled by a hypergeometric distribution,
                    and so the probability of it upperly deviating from the mean by $\zeta$, the event $X_{A,b,A'}$, can be bounded
                    (see~\cite{hypergeometric_tail_inequalities_ending_the_insanity, the_tail_od_the_hypergeometric_distribution})
                    by
                    \[
                        P\parround{X_{A,b,A'}} \leq e^{-2\zeta^2 m}.
                    \]
                ...
                \item Let $B \subseteq G$ be an $\epsilon'$-good set.
                    ...
                    First inequality uses \Cref{rmk:excellence_imply_little_exceptions} for the numerator, as it upper
                    bounds the number of exceptional edges we can and taking the worst case of only
                    $(1 - \epsilon) \parstraight{A}$ exceptional edges per exceptional $b \in B$
                    (considering that $A$ is $\epsilon$-good). \todo{HERE!}
