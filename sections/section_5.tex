\section{The Stable Regularity Lemma} \label{sec:section_5}
    ...
    \subsection{Goodness and excellence} \label{subsec:subsection_5.1}
        ...
        \begin{definition}[Definition 5.2 (1) in~\cite{regularity_lemmas_for_stable_graphs}] \label{def:epsilon_good}
        ...
        \begin{definition}[Definition 5.2 (2) in~\cite{regularity_lemmas_for_stable_graphs}] \label{def:epsilon_excellent}
            Let $G$ be a finite graph with the non-$k_*$-property.
        ...
        \begin{remark}
            ...
            For example, in \Cref{fig:non-monotonic_example}, $A$ is a $\frac{2}{5}$-good set, but $B$ is witnessing
        ...
        \begin{remark} \label{rmk:excellence_imply_little_exceptions}
            ...
            the density (as defined in \Cref{def:density}) of exceptional edges between them is less than $2\epsilon$.
            See \Cref{rmk:density_vs_real_density} to see the relation of this density and the real fraction of exceptional
            edges of the pair.
        \end{remark}

        \begin{remark} \label{rmk:excellence_is_not_monotonic}
            A final important remark, is the fact that differently from most \regular~properties, $\epsilon$-excellence is
            not \say{monotonic}.
            ...
            See \Cref{fig:non-monotonic_example} for a counterexample to the monotonicity of this property.
    ...
    \subsection{Excellent partitions} \label{subsec:subsection_5.2}
        ...
        Similar to \Cref{lem:existance_of_indivisible_sets} in \Cref{sec:section_4}, we prove their existence by assuming
        ...
        \Cref{lem:existance_of_excellent_subsets_fixed_size_choices} is the one we will be using in further
        ...
        \begin{lemma}[Claim 5.4 (I) in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:existance_of_excellent_subsets}
            ...
            \begin{proof}
                ...
                This allows the existence of the $B_\eta$ claimed in \dref{itm:existance_of_excellent_subsets.2}.
                Condition \dref{itm:existance_of_excellent_subsets.4} follows from the definition of $A_{\eta \frown \Partriangle{i}}$ in
                \dref{itm:existance_of_excellent_subsets.3} and the fact that $B_\eta$ is witnessing that $A_\eta$ is not $(\epsilon, \zeta)$-excellent.
                ...
                Finally, by \dref{itm:existance_of_excellent_subsets.3} and \dref{itm:existance_of_excellent_subsets.6}
                ...
                This contradicts the tree bound $k_{**}$ (see \Cref{def:tree_bound}).
            \end{proof}
        \end{lemma}

        \begin{remark}
            The two sequences $\Partriangle{a_\eta \mid \eta \in \parcurly{0,1}^{k_{**}}}$ and $\Partriangle{b_\nu \mid \nu \in \parcurly{0,1}^{<k_{**}}}$,
            constructed in the proof of \Cref{lem:existance_of_excellent_subsets}, are not necessarily disjoint.
        ...
        \begin{lemma}[Claim 5.4 (II) in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:existance_of_excellent_subsets_fixed_size_choices}
            Let $G$ be a finite graph with the non-$k_{*}$-order property.
        ...
        \begin{lemma}[Claim 5.14 (1) in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:existance_of_excellent_partition}
            Let $G$ be a finite graph with the non-$k_{*}$-order property.
            ...
            \begin{proof}
                ...
                Notice that, since $\frac{m_\ell}{m_{\ell-1}} \leq \epsilon$, then by \Cref{rmk:excellence_is_not_monotonic}
                the pair being $\parround{\frac{m_\ell}{m_{\ell-1}}, \epsilon'}$-excellence
                implies it is also $\parround{\epsilon, \epsilon'}$-excellence.
            \end{proof}
        \end{lemma}

        Note that, in \Cref{lem:existance_of_excellent_partition},
        if $n < m_0$ then the statement holds for an empty partition where the remainder is the whole set $A$.
        ...
        \dref{itm:even_partition_of_excellent_parts.2} in
        ...
        pieces of equal size (condition \dref{itm:even_partition_of_excellent_parts.3} in
        \Cref{lem:even_partition_of_excellent_parts}), which still are excellent.
        ...
        Before getting to it, we prove the following calculus result, which we use in the proof of \Cref{clm:ineq_5.13}.
        ...
        \begin{lemma}[Claim 5.13 in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:even_partition_of_excellent_parts}
            Let $G$ be a finite graph with the non-$k_{*}$-order property.
            Then:
            \begin{enumerate}[label={\Roman*}., ref={\Roman*}, font=\rmfamily]
            ...
            \begin{enumerate}[label={\Roman*}., ref={\Roman*}, font=\rmfamily]
                \item The proof of this point uses the hypergeometric distribution.
                    ...
                    Now we want to study the random variable $X$ counting the number of events in
                    $X_{A,b,A'}$ that are satisfied.
                    That is, $X = \sum_{\text{bad } \overline{B}_{A,b}} \mathbbm{1}_{X_{A,b,A'}}$, where $\mathbbm{1}_Y$ is the
                    indicator random variable of the event $Y$.
                    ...
                    \]
                    Following \dref{itm:k_order_propery_bounds_BAbs.2} in \Cref{cor:k_order_propery_bounds_BAbs}, the number of intersections of bad $\overline{B}_{A,b}$'s with $A'$, can be bounded
                    by $m^{k_*}$.
                    ...
                    \]
                    Last inequality follows \Cref{clm:ineq_5.13} using the lower bound
                    $m \geq \frac{1}{\zeta^2}\parround{k_* \log{\frac{1}{\zeta^2}k_*} - \log{\xi}}$.
                ...
                \item Suppose that $A'$ is the subset described in \dref{itm:even_partition_of_excellent_parts.1}.
                    ...
                    \[
                        \parstraight{A' \cap \overline{B}_{A,b}} < \parround{\epsilon + \zeta} \parstraight{A'}
                    \]
                    for all $b \in G$ such that $\parstraight{\overline{B}_{A,b}} \geq \epsilon m$.
                    Thus, given $b \in G$, we have that:
                    \begin{itemize}
                        \item If $\parstraight{\overline{B}_{A,b}} < \epsilon m$, then
                            $\parstraight{\parcurly{a \in A' \mid a R b \not \equiv t(b,A)}} \leq \parstraight{\overline{B}_{A,b}}
                            < \epsilon m < \parround{\epsilon + \zeta} m$.
                        \item If $\parstraight{\overline{B}_{A,b}} \geq \epsilon m$, then
                            $\parstraight{\parcurly{a \in A' \mid a R b \not \equiv t(b,A)}} = \parstraight{A' \cap \overline{B}_{A,b}}
                            < \parround{\epsilon + \zeta} m$.
                    \end{itemize}
                We conclude that $t(b,A) = t(b,A')$ for all $b \in G$.
                \item
                    Let $B \subseteq G$ be an $\epsilon'$-good set.
                    It follows \Cref{rmk:excellence_imply_little_exceptions} that number of exceptional edges
                    ...
                    \end{align}
                    The first equality follows \dref{itm:even_partition_of_excellent_parts.1*}.
                ...
                \item Let $\zeta, \zeta', \epsilon, \epsilon'$ and $r$ be given satisfying the conditions of the statement.
                    ...
                    we can apply statement \dref{itm:even_partition_of_excellent_parts.1}~to get that
                    \[
                        \mathbb{P}\parround{A' \text{ is not } \parround{\epsilon + \zeta'}\text{-good}} < \xi.
                    \]
                    Observe that the hypotheses of statement \dref{itm:even_partition_of_excellent_parts.2}~are satisfied,
        ...
        \begin{remark} \label{rmk:value_for_even_partition_of_excellent_parts}
            In some of the following arguments, we would like to use statement \dref{itm:even_partition_of_excellent_parts.3}~from
            \Cref{lem:even_partition_of_excellent_parts} with $\epsilon' > k \cdot \parround{\epsilon + \zeta}$, for an arbitrarily large $k \in \naturals$.
            ...
            \]
            then statement \dref{itm:even_partition_of_excellent_parts.3}~from
            \Cref{lem:even_partition_of_excellent_parts} holds with $\epsilon' \geq k \cdot \parround{\epsilon + \zeta}$.
        \end{remark}
        ...
        \begin{lemma} \label{lem:existance_of_even_excellent_subpartition}
            ...
            \begin{proof}
                ...
                so statement \dref{itm:even_partition_of_excellent_parts.3} from \Cref{lem:even_partition_of_excellent_parts}
                holds together with $\frac{\epsilon'}{k} \geq \epsilon + \zeta$ and,
                given~\Cref{rmk:excellence_is_not_monotonic}, we are done.
            \end{proof}
        \end{lemma}
        ...
        \begin{lemma}[Claim 5.14 (1A) in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:existance_of_excellent_partition_with_equal_size}
            Let $G$ be a finite graph with the non-$k_{*}$-order property.
        ...
        Notice that our partition is even with a small remainder.
        ...
        \begin{lemma}[Claim 5.14 (2) in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:existance_of_excellent_partition_with_equal_size_and_no_remainder}
            ...
            \begin{proof}
                ...
                This proves that \dref{itm:existance_of_excellent_partition_with_equal_size_and_no_remainder.c} can be satisfied.
            \end{proof}
        \end{lemma}
        ...
        \begin{remark}[Remark 5.14 (3) in~\cite{regularity_lemmas_for_stable_graphs}] \label{rmk:epsilons_proportion_can_be_k}
            Under the hypotheses of \Cref{lem:existance_of_excellent_partition_with_equal_size_and_no_remainder}, if:
            \begin{enumerate}[label={\Roman*}., ref={\Roman*}, font=\rmfamily]
            ...
            \begin{proof}
                Using the notation of the proof of \Cref{lem:existance_of_excellent_partition_with_equal_size_and_no_remainder},
        ...
        \begin{lemma}[Corollary 5.15 in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:resume_of_all_conditions_for_excellent_partitions}
            ...
            \end{enumerate}
            \begin{proof}
                First of all, notice that if condition \dref{itm:resume_of_all_conditions_for_excellent_partitions.b}
                is satisfied, then we also have $m_{**} \geq \frac{3}{\epsilon}$.
                To prove the statement, we use \Cref{lem:existance_of_excellent_partition_with_equal_size_and_no_remainder}
                in the context of \Cref{rmk:epsilons_proportion_can_be_k} with the following parameters: $k = 3$,
                ...
                by \Cref{rmk:excellence_is_not_monotonic} they are also
                $\epsilon'_{\ref{lem:existance_of_excellent_partition_with_equal_size_and_no_remainder}}$-excellent, satisfying
                \dref{itm:resume_of_all_conditions_for_excellent_partitions.ii}.
            \end{proof}
        \end{lemma}
        ...
        \begin{theorem}[Theorem 5.18 in~\cite{regularity_lemmas_for_stable_graphs}] \label{thm:minimal_conditions_for_excellent_partitions}
            Let $k_*$ be given (which by \Cref{thm:tree_implies_order} gives a bound on $k_{**}$).
            ...
            \end{enumerate}
            \begin{proof}
                We aim at applying \Cref{lem:resume_of_all_conditions_for_excellent_partitions}.
                ...
                \begin{enumerate}[label={\Roman*}., ref={\Roman*}, font=\rmfamily]
                    \item \label{itm:minimal_conditions_for_excellent_partitions.a} $m_{**} \in \parsquared{\delta n -1, \delta n}$, where
                        $\delta = \min \parcurly{\frac{\epsilon}{\parround{3 + \epsilon} q^{k_{**}}}, \frac{1}{m + q^{k_{**}}}}$.
                ...
                \dref{itm:minimal_conditions_for_excellent_partitions.2}.
    ...
    \subsection{The Stable Regularity Lemma} \label{subsec:subsection_5.3}
        ...
        regularity (see \Cref{def:regularity}).
        ...
        \begin{lemma}[Lemma 5.17 in~\cite{regularity_lemmas_for_stable_graphs}] \label{lem:excellence_implies_regularity}
            ...
            \begin{proof}
                ...
                \]
                proving \dref{itm:excellence_implies_regularity.1}.
                Similarly,
                \begin{align*}
                ...
                Thus, we conclude that $\parround{A,B}$ is $\epsilon$-regular (statement \dref{itm:excellence_implies_regularity.a})
                and that $d \parround{A',B'}$ is either $< \epsilon$ or $\geq 1 - \epsilon$ (statement \dref{itm:excellence_implies_regularity.b})
        ...
        \begin{remark}
            ...
            It can be derived from a similar argument to that of \Cref{lem:H_like_partition_implies_H_abundance}.
        \end{remark}
        ...
        \begin{theorem}[Theorem 5.19 in~\cite{regularity_lemmas_for_stable_graphs}] \label{thm:existance_of_regular_partitions}
            For every $k_* \in \naturals$ (which by \Cref{thm:tree_implies_order} gives a bound on $k_{**}$) and
            ...
            \end{enumerate}
            \begin{proof}
                If $\epsilon \leq \min \parcurly{\frac{1}{5}, \frac{1}{2^{k_{**}}}}$, then we obtain \dref{itm:existance_of_regular_partitions.1},
                \dref{itm:existance_of_regular_partitions.2} and \dref{itm:existance_of_regular_partitions.3} applying
                \Cref{thm:minimal_conditions_for_excellent_partitions} to $A$ with
                $\epsilon_{\ref{thm:minimal_conditions_for_excellent_partitions}} = \frac{\epsilon^2}{2}$, and then use
                \Cref{lem:excellence_implies_regularity} to get the $\epsilon$-regularity of pairs from the $\frac{\epsilon^2}{2}$-excellence
                of the parts.
